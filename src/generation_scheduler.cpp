...
std::future<GenerationResult> GenerationScheduler::generate(GenerationContext c,std::shared_ptr<IGenerator> g,JobPriority p){if(!g)throw std::invalid_argument("generator must not be null");return pool_.submit(int(p),[c=std::move(c),g=std::move(g)]()mutable{c.stage_seed=deterministicStageSeed(c.world_seed,c.dimension,c.chunk,c.stage);ChunkBuffer chunk(c.chunk);g->generate(c,chunk);auto fp=chunk.fingerprint();return GenerationResult{std::move(chunk),std::string(g->identifier()),fp};});}
std::future<GenerationResult> GenerationScheduler::populate(GenerationContext c,ChunkBuffer chunk,std::shared_ptr<IPopulator> pop,JobPriority p){if(!pop)throw std::invalid_argument("populator must not be null");return pool_.submit(int(p),[this,c=std::move(c),chunk=std::move(chunk),pop=std::move(pop)]()mutable{auto guard=locks_.acquire(c.chunk,pop->radius());c.stage_seed=deterministicStageSeed(c.world_seed,c.dimension,c.chunk,c.stage);pop->populate(c,chunk);auto fp=chunk.fingerprint();return GenerationResult{std::move(chunk),std::string(pop->identifier()),fp};});}
std::future<GenerationResult> GenerationScheduler::populatePipeline(GenerationContext c,ChunkBuffer chunk,
    ...
    return pool_.submit(int(p),[this,c=std::move(c),chunk=std::move(chunk),populators=std::move(populators)]()mutable{
        int radius=0; for(const auto &pop:populators) if(pop){if(pop->radius()<0)throw std::invalid_argument("populator radius must not be negative");radius=std::max(radius,pop->radius());}
        ...
        for(std::size_t i=0;i<populators.size();++i){auto &pop=populators[i];if(!pop)continue;
            c.stage=GenerationStage::Decoration;c.stage_seed=deterministicStageSeed(c.world_seed,c.dimension,c.chunk,c.stage);
            const auto identifier_hash=deterministicStringHash(pop->identifier());
            c.stage_seed^=identifier_hash+0x9e3779b97f4a7c15ULL+(c.stage_seed<<6)+(c.stage_seed>>2);
