...
6. Same-platform relocation/import smoke tests for each native bridge wheel, followed by command and permission verification.
...
The workflow downloads the verified Linux build artifacts, creates `SHA256SUMS.txt`, and creates a published, non-prerelease GitHub Release. Stable `v0.4.8` is explicitly marked as the repository's Latest release.
...
- Verified Linux x64 `endstone_worldgen_studio` wheels
