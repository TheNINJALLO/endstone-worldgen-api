...
```sh
git tag v0.4.8-alpha.1
```

Build only the source/portable assets
with `python scripts/package_compatibility_release.py --output build/compatibility-release`.
The native release procedure below documents the older exact-runtime pipeline.
...
The workflow downloads both verified exact-build artifacts, creates `SHA256SUMS.txt`, and creates a published, non-prerelease GitHub Release. Stable `v0.4.7` is explicitly marked as the repository's Latest release.
